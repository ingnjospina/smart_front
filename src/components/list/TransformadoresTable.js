...
import {deleteTransformador, getTransformadores} from "../../services/transformer.services";
import {EditTransformadorModal} from "../modals/EditTransformadorModal";

export const TransformadoresTable = () => {
    const logout = UseLogout();
    const [transformadores, setTransformadores] = useState([]);
    const [loading, setLoading] = useState(true);
    const [showSpinner, setShowSpinner] = useState(false);
    const [showEditModal, setShowEditModal] = useState(false);
    const [selectedTransformadorId, setSelectedTransformadorId] = useState(null);
    ...
    const fetchTransformadores = async () => {
        try {
            ...
            console.error("Error al obtener los transformadores:", error);
            setTransformadores([]);
            setLoading(false);
            setShowSpinner(false);
            logout.logOut();
    ...
    const handleEdit = (id) => {
        setSelectedTransformadorId(id);
        setShowEditModal(true);
    };

    const handleDelete = async (id) => {
        if (window.confirm("¿Estás seguro de que deseas eliminar este transformador?")) {
            try {
                const user = JSON.parse(window.localStorage.getItem("loggedAppUser")) || {};
                await deleteTransformador(user.token, id);
                setTransformadores(transformadores.filter((transformador) => transformador.idtransformadores !== id));
            } catch (error) {
                console.error("Error al eliminar el transformador:", error);
            }
        }
    };
    ...
    return (
        <div className="container">
                            ...
                            <Table responsive="sm">
                                ...
                                {transformadores.length > 0 ? (
                                    transformadores.map((transformador) => (
                                        <tr key={transformador.idtransformadores}>
                                            <StyledTD>{transformador.idtransformadores}</StyledTD>
                                            <StyledTD>{transformador.nombre}</StyledTD>
                                            <StyledTD>{transformador.descripcion}</StyledTD>
                                            <StyledTD>
                                                <a href='#' onClick={() => handleEdit(transformador.idtransformadores)}
                                                ...
                                                <a href='#'
                                                   onClick={() => handleDelete(transformador.idtransformadores)}
                                                   style={{color: "red"}}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26"
                                                         viewBox="0 0 26 26" fill="none">
                                                        <path
                                    ...
                                    <tr>
                                        <StyledTD colSpan="4">No hay transformadores disponibles.</StyledTD>
                                    </tr>
                                )}
                                </tbody>
                            </Table>
            ...
            <EditTransformadorModal
                show={showEditModal}
                handleClose={() => setShowEditModal(false)}
                transformadorId={selectedTransformadorId}
                onUpdate={fetchTransformadores} // Recargar la lista después de actualizar
            />
        </div>
    );
};

export default TransformadoresTable;
